...
ap.add_argument("-y", "--yolo", default='car_color_classifier_yolo3_python/yolo-coco',
...
image = cv2.imread(args["image"]) #taken out to read from parameter
(H, W) = image.shape[:2]
...
if len(idxs) > 0:
	...
	for i in idxs.flatten():
		...
		if classIDs[i] == 2:
			...
			text = "{}: {:.4f}".format(result[0]['color'], float(result[0]['prob'])) #result[0]['color'] is color in str
